...
- (void)onLogoutCompletion
{
    ...
    [[TCILiveManager sharedInstance] onLogoutCompletion];
#endif
...
- (void)configHost:(TIMLoginParam *)param completion:(CommonVoidBlock)block
{
    ...
    [[TCILiveManager sharedInstance] startContextWith:param completion:^(BOOL succ, NSError *err) {
        DebugLog(@"succ : %d, err = %@", succ, err);
        if (block)
        {
            block();
        }
