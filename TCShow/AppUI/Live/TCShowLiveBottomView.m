...
{
    if ([[TCILiveManager sharedInstance] isFrontCamera])
    {
    ...
    // 是否有摄像头
    // 打开LED
    [[TCILiveManager sharedInstance] turnOnFlash:btn.selected];
}
...
- (void)onSwitchCamera:(MenuButton *)btn
{
    ...
    [mgr switchCamera:mgr.isFrontCamera ? CameraPosBack : CameraPosFront  complete:^(BOOL succ, QAVResult result) {
        btn.enabled = YES;
    }];
}
...
- (void)onBeautyChanged:(CGFloat)value
{
    ...
    [[TCILiveManager sharedInstance].avContext.videoCtrl inputBeautyParam:be];
}
...
- (void)onWhiteChanged:(CGFloat)value
{
    ...
    [[TCILiveManager sharedInstance].avContext.videoCtrl inputWhiteningParam:be];
}
...
- (void)onSwtichMic:(MenuButton *)btn
{
    ...
    BOOL succ = [[TCILiveManager sharedInstance] enableMic:btn.selected];
...
{
    btn.selected = !btn.selected;
    BOOL succ = [[TCILiveManager sharedInstance] enableSpeaker:btn.selected];
...
{
    if ([[TCILiveManager sharedInstance] isHostLive])
    {
        return;
    }
...
- (void)addFunc:(NSInteger)funcid
{
    ...
    switch (funcid)
    {
        ...
        case EFunc_Mic:
        {
            ...
            menuBtn.selected = [[TCILiveManager sharedInstance].room.config autoEnableMic];
        }
            break;
        case EFunc_Speaker:
        {
...
#if kSupportIMMsgCache
- (void)showLikeHeart:(TCILAVIMCache *)cache
{
    if (_nonPureFunc == 0)
    {
        if (cache.count)
...
- (void)changeTo:(TCShowLiveBottomType)type isHost:(BOOL)isHost isPure:(BOOL)isPure
{
    if (isHost)
    {
        switch (type)
        {
            case ETCShowLiveBottom_Host:
            {
                if (isPure)
                {
                    [self showFunc:EFunc_NonPure];
                ...
                else
                {
                    ...
                    NSInteger func =  _lastFunc;
                    func = func | EFunc_LED;
                    func = func | EFunc_Beauty;
                    func = func | EFunc_White;
                    func = func | EFunc_Mic;
                    func = func | EFunc_Camera;
                    func = func | EFunc_Speaker;
                    func = func & ~EFunc_MSG;
            ...
            case ETCShowLiveBottom_Interact:
            {
                if (isPure)
                {
                    [self showFunc:EFunc_NonPure];
                ...
                {
                    _lastFunc = EFunc_MultiAll | EFunc_Pure;
                    [self showFunc:_lastFunc];
                }
            }
                break;
                
            case ETCShowLiveBottom_Guest:
                // 主播端不显示观众底部
                break;
            default:
                break;
    ...
    else
    {
        switch (type)
        {
            case ETCShowLiveBottom_Host:
            {
                ...
                {
                    _lastFunc = EFunc_LocalAll & ~EFunc_LED & ~EFunc_Camera & ~EFunc_Beauty & ~EFunc_White & ~EFunc_Mic & ~EFunc_Share & ~EFunc_NonPure;
                    [self showFunc:_lastFunc];
                }
            }
            ...
            case ETCShowLiveBottom_Interact:
            {
                ...
                {
                    _lastFunc = EFunc_LocalAll & ~EFunc_LED & ~EFunc_Camera & ~EFunc_Beauty & ~EFunc_White & ~EFunc_Mic ;
                    _lastFunc |= EFunc_Multi_SwitchToMain;
                    [self showFunc:_lastFunc];
                }
            }
