...
- (void)startLive
{
    ...
    TCILiveRoom *room = [[TCILiveRoom alloc] initLiveWith:liveRoom.avRoomId liveHost:[host imUserId] curUserID:[host imUserId] roomControlRole:@"LiveHost"];
    room.config.autoMonitorForeBackgroundSwitch = NO;
    
    [[HUDHelper sharedInstance] syncLoading:@"开始直播"];
    ...
    [[TCILiveManager sharedInstance] enterRoom:room imChatRoomBlock:^(BOOL succ, NSString *groupID, NSError *err) {
        ...
        {
            [[HUDHelper sharedInstance] syncStopLoadingMessage:@"创建直播聊天室出错"];
        }
        ...
        if (succ)
        {
            ...
            [ws.navigationController pushViewController:pcvc animated:YES];
//            [ws presentViewController:pcvc animated:YES completion:nil];
        }
        else
        {
            [[HUDHelper sharedInstance] syncStopLoadingMessage:@"创建直播房间出错"];
        }
